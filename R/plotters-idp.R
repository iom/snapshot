...
plot_idp_pt <- function(hero,
                        basesize,
                        font,
                        title = paste0(
                          "Novos deslocamentos internos, ",
                          plot_data("idp", hero)$range |>
                            paste(collapse = "\u2013")
                        )) {
  ...
  if (nrow(data) > 0) {
    ...
    df_agg <- summarise(data, n = sum(.data$n), .by = .data$t) |>
      filter(.data$n > 0)
    axis <- set_axis(df_agg$n, "Deslocamentos", lang = "pt")
...
plot_idcause_pt <- function(hero,
                            basesize,
                            font,
                            title = paste0(
                              "Causas dos novos deslocamentos internos\ndevido a impactos ambientais, ",
                              plot_data("idcause")$range |>
                                paste(collapse = "\u2013")
                            )) {

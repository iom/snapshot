...
plot_idpmap_pt <- function(hero,
                           basesize,
                           font,
                           title = "Mapa de novos deslocamentos internos,\n2020\u20132024") {
  ...
  if (nrow(data) > 0) {
    ...
    plot <- base_map + 
      ...
      # Scales
      scale_shape_manual(name = "Tipo", values = c(21, 23)) +
      scale_fill_steps2(
        name = "Ano", 
        breaks = t0:t1,
        high = pal("blues", 2),
...
plot_mmpmap_pt <- function(hero,
                           basesize,
                           font,
                           title = "Mapa de migrantes internacionais mortos ou\ndesaparecidos, 2014\u20132024") {
  ...
  if (nrow(data) > 0) {
    ...
    plot <- base_map + 
      ...
      # Scales
      scale_fill_manual(name = "Causa", values = causes) +
      scale_size_continuous(
        name = "Pessoas",
        limits = c(min(df$n), max(df$n)),
        breaks = scale$breaks,
